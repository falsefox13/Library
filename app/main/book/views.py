...
def edit(book_id):
    ...
    return render_template("book_edit.html", form=form, book=book, title=u"Edit books")
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'Successfully deleted the book', 'info')
...
def put_back(book_id):
    ...
    db.session.add(the_book)
    db.session.commit()
    flash(u'Successfully restored the book,', 'info')
    return redirect(request.args.get('next') or url_for('book.detail', book_id=book_id))
